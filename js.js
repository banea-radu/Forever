...
function FilterFromDB() {
	...
	get(child(dbRef, 'Frvr/Clienti')).then((snapshot) => {
  		snapshot.forEach(function(ChildSnapshot) {
			var input, filter, filter_nueavatar, txtValue;
			input = document.getElementById("myInput");
			filter = input.value.toUpperCase();
			filter_nueavatar = "nu e avatar";
			...
			if (txtValue.toUpperCase().indexOf(filter_nueavatar.toUpperCase()) == -1) { //continue only if text not found
